  ...
  <div>
    <div class="container">
        <div class="navbar">
            ...
            <div class="navBtn" v-on:click="clickNavBarBtn">
           ...
           <div class="navBtnInfo">
                <div v-if="isLogin == true" style="display: flex; flex-direction: column; justify-content: center; align-items: center;">
                    <a>欢迎回来，拉布拉马</a>
                    <button class="logoutBtn" v-on:click="logoutUser">退出登录</button>
                </div>

                <div v-if="isLogin == false" style="display: flex; flex-direction: column; justify-content: center; align-items: center;">
                    <a>请先注册或登录</a>
                    <input class="userInput" type="text" v-model="username">
                    <input class="userInput" type="text" v-model="password">
                    <button class="userBtn" v-on:click="loginUser">登陆</button>
                    <button class="userBtn" v-on:click="registerUser">注册</button>
                    <a class="loginTip">{{loginTip}}</a>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        clickNavBarBtn: function(){
            var btn = this.getClass("navBtn", 0);
            var info = this.getClass("navBtnInfo", 0);
            ...
            if(this.username == ""){
                ...
                axios.get(apiUrl + '/api/loginUser', {
                    ...
                    switch(res.data.type){
                        ...
                        case "correct":
                            var navInfo = this.getClass("navBtnInfo", 0);
